...
def clean():
    ...
    try:
        for folder in folders_list:
            if (time.time() - os.stat(folder).st_ctime) / 3600 > 0.003:
...
def favicon(): 
    return send_from_directory(os.path.join(app.root_path, 'static'), '-Cropwatch-Icon.png')
